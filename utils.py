...
def calculate_tips_distribution(total_amount: float) -> dict:
    ...
    До 6000: 30% Сбербанк, 5% Яндекс Пэй, 5% Бизнес, 30% Жизнь, 20% Хотелки, 10% Обучение
    Свыше 6000: остаток распределяется как 30% Сбербанк, 10% Акции, 30% Хотелки, 30% Жизнь
    """
    distribution = {
        'Сбербанк': 0,
        'Яндекс Пэй': 0,
    ...
    if total_amount <= 6000:
        # До 6000 - первая схема на всю сумму
        distribution['Сбербанк'] = total_amount * 0.30
        distribution['Яндекс Пэй'] = total_amount * 0.05
        distribution['Бизнес'] = total_amount * 0.05
        distribution['Жизнь'] = total_amount * 0.30
        distribution['Хотелки'] = total_amount * 0.20
        distribution['Обучение'] = total_amount * 0.10
    else:
        # Первые 6000 по первой схеме
        distribution['Сбербанк'] = 6000 * 0.30
        distribution['Яндекс Пэй'] = 6000 * 0.05
        distribution['Бизнес'] = 6000 * 0.05
        distribution['Жизнь'] = 6000 * 0.30
        distribution['Хотелки'] = 6000 * 0.20
        distribution['Обучение'] = 6000 * 0.10
        
        # Остаток по второй схеме
        remainder = total_amount - 6000
        distribution['Сбербанк'] += remainder * 0.30
        distribution['Акции'] = remainder * 0.10
        distribution['Хотелки'] += remainder * 0.30
        distribution['Жизнь'] += remainder * 0.30
